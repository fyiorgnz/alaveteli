...
class UserController < ApplicationController
  ...
  def show
    ...
    unless @display_user
      fail ActiveRecord::RecordNotFound.new('user not found, url_name=' + params[:url_name])
    end
    ...
    @undescribed_requests = @display_user.get_undescribed_requests if @is_you
  ...
  def signchangepassword
    if @user && ((!session[:user_circumstance]) || (session[:user_circumstance] != 'change_password'))
    ...
    if params[:submitted_signchangepassword_send_confirm]
      ...
      if user_signchangepassword
        ...
        url += '?pretoken=' + params[:pretoken] if params[:pretoken]
      ...
      fail 'internal error' unless (session[:user_circumstance] == 'change_password')
      ...
      if params[:submitted_signchangepassword_do]
        ...
        if !@user.valid?
          ...
          if params[:pretoken] && !params[:pretoken].empty?
  ...
  def river
    @results = @user.nil? ? [] : @user.track_things.collect do |thing|
      perform_search([InfoRequestEvent], thing.track_query, thing.params[:feed_sortby], nil).results
    end.flatten.sort { |a, b| b[:model].created_at <=> a[:model].created_at }.first(20)
  end
  ...
  def clear_profile_photo
    fail 'Can only clear profile photo from POST request' unless request.post?
  ...
  def get_profile_photo
    ...
    unless @display_user.profile_photo
      fail ActiveRecord::RecordNotFound.new('user has no profile photo, url_name=' + params[:url_name])
  ...
  def work_out_post_redirect
    ...
    params[:r] = '/' if !params[:r] && !params[:token]
