...
class TrackController < ApplicationController
  ...
  def track_request
    ...
    redirect_to request_url(@info_request) if track_set
  end
  ...
  def track_list
    ...
    if @view == 'recent' || @view.nil? # the blank one for backwards compatibility
      ...
      fail 'unknown request list view ' + @view.to_s
    end

    return atom_feed_internal if params[:feed] == 'feed'

    redirect_to request_list_url(view: @view) if track_set
  end

  # Track all updates to a particular public body
  def track_public_body
    @public_body = PublicBody.find_by_url_name_with_historic(params[:url_name])
    fail ActiveRecord::RecordNotFound.new('None found') if @public_body.nil?
    ...
    redirect_to public_body_url(@public_body) if track_set
  end

  # Track a user
  def track_user
    @track_user = User.find_by_url_name(params[:url_name])
    fail ActiveRecord::RecordNotFound.new('No such user') if @track_user.nil?
    @track_thing = TrackThing.create_track_for_user(@track_user)

    return atom_feed_internal if params[:feed] == 'feed'

    redirect_to user_url(@track_user) if track_set
  end
  ...
  def atom_feed
    ...
    if @track_thing.track_medium != 'feed'
      fail 'can only view feeds for feed tracks, not email ones'
    end
  ...
  def update
    ...
    if new_medium == 'delete'
      ...
      fail 'new medium not handled ' + new_medium
    end
  end
