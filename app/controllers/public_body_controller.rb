...
class PublicBodyController < ApplicationController
  ...
  def list
    ...
    if @tag.nil? || @tag == 'all'
      ...
      category_list = PublicBodyCategory.get.tags.map { |c| %('#{ c }') }.join(',')
  ...
  def simplify_stats_for_graphs(data,
    ...
    result.update Hash[data.select do |key, _value|
      %w(y_values y_max totals cis_below cis_above).include? key
    end]
  ...
  def statistics
    unless AlaveteliConfiguration.public_body_statistics_page
      fail ActiveRecord::RecordNotFound.new('Page not enabled')
    end
  ...
  def get_public_body_list_translated_condition(table, first_letter = false, locale = nil)
    ...
    result += " AND #{table}.first_letter = ?" if first_letter
    result += " AND #{table}.locale = ?" if locale
    result
  end
end
