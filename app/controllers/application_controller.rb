...
class ApplicationController < ActionController::Base
  ...
  def anonymous_cache(time)
    expires_in time, public: true if session[:user_id].nil?
  end
  ...
  def record_memory
    ...
    if record_memory
      ...
      rss_before_action = Regexp.last_match(1).to_i
      yield
      File.read("/proc/#{Process.pid}/status").match(/VmRSS:\s+(\d+)/)
      rss_after_action = Regexp.last_match(1).to_i
  ...
  def test_code_redirect_by_email_token(token, controller_example_group)
    ...
    fail 'bad token in test code email' if post_redirect.nil?
  ...
  def foi_fragment_cache_path(param)
    ...
    # characters
    File.join(File.split(path).map { |x| x[0...max_file_length] })
  end
  ...
  def check_in_post_redirect
    if params[:post_redirect] && session[:post_redirect_token]
  ...
  def order_to_sort_by(sortby)
    if sortby.nil?
      ...
      fail 'Unknown sort order ' + @sortby
    end
  end
  ...
  def country_from_ip
    country = ''
    unless AlaveteliConfiguration.gaze_url.empty?
      begin
