...
class AdminPublicBodyController < AdminController
  ...
  def mass_tag_add
    ...
    if params[:new_tag] && params[:new_tag] != ''
      if params[:table_name] == 'exact'
        ...
        fail 'Unknown table_name ' + params[:table_name]
      end
      for body in bodies
        ...
        if errors.size == 0
          if dry_run_only
            ...
            errors, notes = PublicBody.import_csv(csv_contents,
                                                  ...
                                                  admin_current_user,
                                                  I18n.available_locales)
            fail 'dry run mismatched real run' if errors.size != 0
  ...
  def store_csv_data(csv_contents)
    tempfile_name = "csv_upload-#{Time.now.strftime('%Y%m%d')}-#{SecureRandom.random_number(10_000)}"
  ...
  def retrieve_csv_data(tempfile_name)
    unless /csv_upload-\d{8}-\d{1,5}/.match(tempfile_name)
      fail "Invalid filename in upload_csv: #{tempfile_name}"
    end
    tempfile_path = File.join(Dir.tmpdir, tempfile_name)
    unless File.exist?(tempfile_path)
      fail "Missing file in upload_csv: #{tempfile_name}"
    end
