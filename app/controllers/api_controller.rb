class ApiController < ApplicationController
  ...
  def show_request
    ...
    fail PermissionDenied if @request.public_body_id != @public_body.id
  ...
  def body_request_events
    ...
    fail PermissionDenied.new("#{@public_body.id} != #{params[:id]}") if @public_body.id != params[:id].to_i
    ...
    if feed_type == 'atom'
      ...
      fail ActiveRecord::RecordNotFound.new("Unrecognised feed type: #{feed_type}")
    end
  end
  ...
  def check_api_key
    fail PermissionDenied.new("Missing required parameter 'k'") if params[:k].nil?
    @public_body = PublicBody.find_by_api_key(params[:k].gsub(' ', '+'))
    fail PermissionDenied if @public_body.nil?
  end
