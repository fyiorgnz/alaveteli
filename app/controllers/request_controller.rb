...
class RequestController < ApplicationController
  before_action :check_read_only, only: [:new, :show_response, :describe_state, :upload_response]
  ...
  def select_authorities
    ...
    respond_to do |format|
      ...
      format.json do
        if @search_bodies
          render json: @search_bodies.results.map{ |result|
  ...
  def show
    ...
    I18n.with_locale(@locale) do
      ...
      return render_hidden unless @info_request.user_can_view?(authenticated_user)
  ...
  def details
    ...
    @info_request = InfoRequest.find_by_url_title!(params[:url_title])
    return render_hidden unless @info_request.user_can_view?(authenticated_user)
    @columns = %w(id event_type created_at described_state last_described_at calculated_state)
  end
  ...
  def similar
    ...
    if @page > MAX_RESULTS / PER_PAGE
      fail ActiveRecord::RecordNotFound.new("Sorry. No pages after #{MAX_RESULTS / PER_PAGE}.")
    end
    @info_request = InfoRequest.find_by_url_title!(params[:url_title])
    fail ActiveRecord::RecordNotFound.new('Request not found') if @info_request.nil?
  ...
  def list
    ...
    @view = params[:view]
    @locale = locale_from_params
    @page = get_search_page_from_params unless @page # used in cache case, as perform_search sets @page as side effect
    @per_page = PER_PAGE
  ...
  def describe_state
    ...
    if %w(error_message requires_admin).include?(described_state) && message.nil?
...
{{date_response_required_by}}</strong>.</p>", date_response_required_by: view_context.simple_date(info_request.date_response_required_by))
    ...
    when 'gone_postal'
      ...
      if @@custom_states_loaded
        ...
        fail "unknown calculate_status #{info_request.calculate_status}"
      end
    end
  end
  ...
  def show_response
    ...
    if (!@incoming_message.nil?) && @info_request != @incoming_message.info_request
      fail ActiveRecord::RecordNotFound.new("Incoming message #{@incoming_message.id} does not belong to request #{@info_request.id}")
    end
    ...
    unless OutgoingMailer.is_followupable?(@info_request, @incoming_message)
      fail 'unexpected followupable inconsistency' if @info_request.public_body.is_requestable?
    ...
    unless authenticated_as_user?(@info_request.user,
  ...
  def authenticate_attachment
    # Test for hidden
    incoming_message = IncomingMessage.find(params[:incoming_message_id])
    fail ActiveRecord::RecordNotFound.new('Message not found') if incoming_message.nil?
    unless incoming_message.info_request.user_can_view?(authenticated_user)
      @info_request = incoming_message.info_request # used by view
      return render_hidden
    end
    unless incoming_message.user_can_view?(authenticated_user)
  ...
  def get_attachment_as_html
    ...
    if @files_can_be_cached != true
      fail ActiveRecord::RecordNotFound.new('Attachment HTML not found.')
    end
  ...
  def get_attachment_internal(html_conversion)
    ...
    if @incoming_message.info_request_id != params[:id].to_i
      ...
      fail ActiveRecord::RecordNotFound.new(message)
    end
    ...
    # check permissions
    fail 'internal error, pre-auth filter should have caught this' unless @info_request.user_can_view?(authenticated_user)
    ...
    if @attachment.display_filename != @original_filename && @attachment.old_display_filename != @original_filename
      ...
      fail ActiveRecord::RecordNotFound.new(msg)
    end
  ...
  def upload_response
    ...
    I18n.with_locale(@locale) do
      ...
      return unless authenticated?(@reason_params)
  ...
  def download_entire_request
    ...
    I18n.with_locale(@locale) do
      ...
      if authenticated?(
        ...
        return render_hidden unless @info_request.user_can_view?(@user)
  ...
  def check_batch_requests_and_user_allowed
    unless AlaveteliConfiguration.allow_batch_requests
      fail RouteNotFound.new('Page not enabled')
    end
    unless authenticated?(
    ...
    unless @user.can_make_batch_requests?
  ...
  def render_new_compose(batch)
    ...
    unless batch
      ...
      if params[:url_name]
        if params[:url_name].match(/^[0-9]+$/)
          ...
          fail ActiveRecord::RecordNotFound.new('None found') if public_body.nil? # TODO: proper 404
      ...
      unless params[:info_request][:public_body]
    ...
    @info_request.is_batch_request_template = true if batch
    params[:info_request_id] = @info_request.id
    params[:outgoing_message] = {} unless params[:outgoing_message]
  ...
  def render_new_preview
    ...
    flash.now[:error] = message.html_safe unless message.empty?
