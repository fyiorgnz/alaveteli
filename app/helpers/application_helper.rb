...
module ApplicationHelper
  ...
  def form_tag_id(object_name, method_name, locale = nil)
    if locale.nil?
      ...
      return "#{sanitized_object_name(object_name.to_s)}_#{sanitized_method_name(method_name.to_s)}__#{locale}"
        end
  end
  ...
  def is_admin?
    !session[:using_admin].nil? || (!@user.nil? && @user.super?)
  end

  def cache_if_caching_fragments(*args, &_block)
