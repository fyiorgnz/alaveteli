...
class RawEmail < ActiveRecord::Base
  ...
  def directory
    if request_id.empty?
      fail 'Failed to find the id number of the associated request: has it been saved?'
    end
  ...
  def filepath
    if incoming_message_id.empty?
      fail 'Failed to find the id number of the associated incoming message: has it been saved?'
    end
