...
class OutgoingMessage < ActiveRecord::Base
  ...
  validates_inclusion_of :status, in: %w(ready sent failed)
  validates_inclusion_of :message_type, in: %w(initial_request followup)
  ...
  default_url_options[:protocol] = 'https' if AlaveteliConfiguration.force_ssl
  ...
  def set_signature_name(name)
    ...
    self.body = raw_body + name if raw_body == get_default_message
  end
  ...
  def sendable?
    if status == 'ready'
      if message_type == 'initial_request'
        ...
        fail "Message id #{id} has type '#{message_type}' which cannot be sent"
      end
    elsif status == 'sent'
      fail "Message id #{id} has already been sent"
    else
      fail "Message id #{id} not in state for sending"
    end
  end

  # An admin function
  def prepare_message_for_resend
    if %w(initial_request followup).include?(message_type) && status == 'sent'
      self.status = 'ready'
    else
      fail "Message id #{id} has type '#{message_type}' status " \
  ...
  delegate :purge_in_cache, to: :info_request
  ...
  def format_of_body
    if body.empty? || body =~ /\A#{Regexp.escape(get_salutation)}\s+#{Regexp.escape(get_signoff)}/ || body =~ /#{Regexp.escape(get_internal_review_insert_here_note)}/
      if message_type == 'followup'
        if what_doing == 'internal_review'
          ...
          fail "Message id #{id} has type '#{message_type}' which validate can't handle"
      end
    end
    ...
    if what_doing.nil? || !%w(new_information internal_review normal_sort).include?(what_doing)
