...
class FoiAttachment < ActiveRecord::Base
  ...
  def directory
    ...
    File.join(base_dir, hexdigest[0..2])
  end

  def filepath
    File.join(directory, hexdigest)
  end
  ...
  def body
    if @cached_body.nil?
      ...
      delay = 1
      begin
        binary_data = File.open(filepath, 'rb') { |file| file.read }
  ...
  def extra_note
    ...
    if @content_type == 'message/delivery-status'
      ...
      dsn = Regexp.last_match(1)
      dsn_part = 'X.' + Regexp.last_match(2)
  ...
  def display_filename
    ...
    unless incoming_message.nil?
      incoming_message.info_request.apply_censor_rules_to_text!(filename)
    end
