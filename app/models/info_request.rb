...
class InfoRequest < ActiveRecord::Base
  ...
  validates_format_of :title, with: /[ёЁа-яА-Яa-zA-Zà-üÀ-Ü]/iu,
                              ...
                              if: proc { |info_request| !info_request.title.nil? && !info_request.title.empty? }
  ...
  validates_presence_of :public_body_id, unless: proc { |info_request| info_request.is_batch_request_template? }
  ...
  validates_inclusion_of :prominence, in: %w(normal backpage hidden requester_only)
  ...
  def self.enumerate_states
    ...
    states += InfoRequest.theme_extra_states if @@custom_states_loaded
    states
  end
  ...
  def must_be_valid_state
    errors.add(:described_state, 'is not a valid state') unless
  ...
  def update_url_title
    ...
    until InfoRequest.find_by_url_title(unique_url_title,
  ...
  def law_used_full
    if law_used == 'foi'
      ...
      fail "Unknown law used '" + law_used + "'"
    end
  end
  ...
  def find_existing_outgoing_message(body)
    ...
    if ActiveRecord::Base.connection.adapter_name == 'PostgreSQL'
      ...
      return outgoing_messages.find(:first, conditions: ["regexp_replace(outgoing_messages.body, '[[:space:]]', '', 'g') = regexp_replace(?, '[[:space:]]', '', 'g')", body])
    else
      # For other databases (e.g. SQLite) not the end of the world being space-sensitive for this check
      return outgoing_messages.find(:first, conditions: ['outgoing_messages.body = ?', body])
    end
  end
  ...
  def already_received?(email, _raw_email_data)
    ...
    fail 'No message id for this message' if message_id.nil?
  ...
  def receive(email, raw_email_data, override_stop_new_responses = false, rejected_reason = '')
    unless override_stop_new_responses
      ...
      if allow_new_responses_from == 'nobody'
        ...
        if sender_email.nil?
          ...
          for row in who_can_followup_to
            request_domain = PublicBody.extract_domain_from_email(row[1])
            allow = true if request_domain == sender_domain
          end
        end
      else
        fail "Unknown allow_new_responses_from '" + allow_new_responses_from + "'"
      end

      unless allow
        if handle_rejected_responses == 'bounce'
          RequestMailer.stopped_responses(self, email, raw_email_data).deliver unless is_external?
        ...
        # new incoming messages there as well as this script)
            ...
            fail "Unknown handle_rejected_responses '" + handle_rejected_responses + "'"
        end
        return
      end
    end
    ...
    ActiveRecord::Base.transaction do
      ...
      raw_sql = "SELECT * FROM info_requests WHERE id = #{id} LIMIT 1 FOR UPDATE"
      ...
      unless rejected_reason.empty?
        params[:rejected_reason] = rejected_reason.to_str
      end
      log_event('response', params)
      self.save!
    end
    info_request_events.each(&:xapian_mark_needs_index) # for the "waiting_classification" index
    RequestMailer.new_response(self, incoming_message).deliver unless is_external?
  end
  ...
  def self.holding_pen_request
    ...
    if ir.nil?
      ...
      om = OutgoingMessage.new(status: 'ready',
  ...
  def requires_admin?
    %w(requires_admin error_message attention_requested).include?(described_state)
  end
  ...
  def calculate_event_states
    ...
    for event in info_request_events.reverse
      ...
      if curr_state.nil?
        curr_state = event.described_state unless event.described_state.nil?
      end
      ...
      if !curr_state.nil? && event.event_type == 'response'
        ...
        curr_state = nil
      elsif !curr_state.nil? && (%w(edit status_update).include? event.event_type)
  ...
  def last_event_forming_initial_request
    ...
    if last_sent.nil?
      fail 'internal error, last_event_forming_initial_request gets nil for request ' + id.to_s + ' outgoing messages count ' + outgoing_messages.size.to_s + ' all events: ' + info_request_events.to_yaml
    end
    last_sent
  end
  ...
  def public_response_events
    info_request_events.select { |e| e.response? && e.incoming_message.all_can_view? }
  end
  ...
  def initial_request_text
    ...
    excerpt = outgoing_messages[0].get_text_for_indexing
    excerpt
  end
  ...
  def index_of_last_described_event
    ...
    events.each_index do |i|
      ...
      m = events[revi]
      return revi unless m.described_state.nil?
    end
    nil
  end
  ...
  def fully_destroy
    track_things.each do |track_thing|
      track_thing.track_things_sent_emails.each(&:destroy)
      track_thing.destroy
    end
    user_info_request_sent_alerts.each(&:destroy)
    info_request_events.each do |info_request_event|
      info_request_event.track_things_sent_emails.each(&:destroy)
      info_request_event.destroy
    end
    mail_server_logs.each(&:destroy)
    outgoing_messages.each(&:destroy)
    incoming_messages.each(&:destroy)
    destroy
  end
  ...
  def magic_email(prefix_part)
    fail 'id required to make magic' unless id
  ...
  def who_can_followup_to(skip_message = nil)
    ...
    for incoming_message in incoming_messages.reverse
      ...
      next unless incoming_message.all_can_view?
  ...
  def applicable_censor_rules
    ...
    applicable_rules << user.censor_rules if user && !user.censor_rules.empty?
  ...
  def all_can_view?
    return true if %w(normal backpage).include?(prominence)
    false
  end

  def all_can_view_all_correspondence?
    all_can_view? &&
      incoming_messages.all?(&:all_can_view?) &&
      outgoing_messages.all?(&:all_can_view?)
  end
  ...
  def update_counter_cache
    ...
    [['info_requests_not_held_count', { described_state: 'not_held' }],
     ['info_requests_successful_count', { described_state: %w(successful partially_successful) }],
  ...
  def set_defaults
    ...
    # FOI or EIR?
    self.law_used = 'eir' if !public_body.nil? && public_body.eir_only?
  end
