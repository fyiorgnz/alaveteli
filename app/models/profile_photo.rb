...
class ProfilePhoto < ActiveRecord::Base
  ...
  def convert_image
    ...
    if !draft && (image.columns != WIDTH || image.rows != HEIGHT)
      ...
      image.crop!(x.to_i, y.to_i, w.to_i, h.to_i) if w && h
  ...
  def data_and_draft_checks
    ...
    if draft && user_id
      fail 'Internal error, draft pictures must not have a user'
    end

    fail 'Internal error, real pictures must have a user' if !draft && !user_id
  end
