...
class InfoRequestEvent < ActiveRecord::Base
  ...
  def must_be_valid_state
    if !described_state.nil? && !InfoRequest.enumerate_states.include?(described_state)
  ...
  def latest_variety
    for event in info_request.info_request_events.reverse
      return event.variety if !event.variety.nil? && !event.variety.empty?
    end
  end

  def latest_status
    for event in info_request.info_request_events.reverse
      if !event.calculated_state.nil? && !event.calculated_state.empty?
  ...
  def title
    return info_request.title if event_type == 'sent'
    ''
  end
  ...
  def filetype
    if event_type == 'response'
      if incoming_message.nil?
        fail "event type is 'response' but no incoming message for event id #{id}"
      end
  ...
  def visible
    return comment.visible if event_type == 'comment'
    true
  end
  ...
  def params=(params)
    ...
    unless params[:incoming_message_id].nil?
      self.incoming_message_id = params[:incoming_message_id]
    end
    unless params[:outgoing_message_id].nil?
      self.outgoing_message_id = params[:outgoing_message_id]
    end
    self.comment_id = params[:comment_id] unless params[:comment_id].nil?
    self.params_yaml = params.to_yaml
  end

  def params
    YAML.load(params_yaml)
  end
  ...
  def params_yaml_as_html
    ...
    for key, value in params
      key = key.to_s
      if key.match(/^old_(.*)$/)
        old_params[Regexp.last_match(1)] = value
    ...
    for key, value in other_params
      ...
      ret += CGI.escapeHTML(value.to_s.strip)
      ret += '<br>'
    end
    ret
  end
  ...
  def is_incoming_message?
    !incoming_message_selective_columns('incoming_messages.id').nil?
  end
  ...
  def display_status
    ...
    fail _('display_status only works for incoming and outgoing messages right now')
  end
  ...
  def json_for_api(deep, snippet_highlight_proc = nil)
    ...
    unless snippet_highlight_proc.nil?
