...
class PublicBody < ActiveRecord::Base
  ...
  def calculate_cached_fields(t)
    ...
    short_long_name = t.short_name if t.short_name && !t.short_name.empty?
  ...
  def set_api_key
    self.api_key = SecureRandom.base64(33) if api_key.nil?
  end
  ...
  def self.find_by_url_name_with_historic(name)
    ...
    old = old.map(&:public_body_id)
    old = old.uniq
    # Maybe return the first one, so we show something relevant,
    # rather than throwing an error?
    fail "Two bodies with the same historical URL name: #{name}" if old.size > 1
  ...
  def not_requestable_reason
    if self.defunct?
      ...
      fail 'not_requestable_reason called with type that has no reason'
    end
  end
  ...
  class Version
    ...
    def compare(previous = nil)
      if previous.nil?
        ...
        v = self
        changes = self.class.content_columns.inject([]) do|memo, c|
  ...
  def short_or_long_name
    if short_name.nil? || short_name.empty?   # 'nil' can happen during construction
      name.nil? ? '' : name
    else
      short_name
    end
  ...
  def self.internal_admin_body
    ...
    when matching_pbs.length == 1 then
      ...
      fail "Multiple public bodies (#{matching_pbs.length}) found with url_name 'internal_admin_authority'"
    end
  end
  ...
  def self.import_csv_from_file(csv_filename, tag, tag_behaviour, dry_run, editor, available_locales = [])
    ...
    begin
      ActiveRecord::Base.transaction do
        ...
        fail ImportCSVDryRun if dry_run || errors.size > 0
      end
  ...
  def self.extract_domain_from_email(email)
    ...
    # take lower case
    ret = Regexp.last_match(1).downcase
  ...
  def purge_in_cache
    info_requests.each(&:purge_in_cache)
  end
  ...
  def self.get_request_totals(n, highest, minimum_requests)
    ...
    y_values = public_bodies.map(&:info_requests_count)
    {
  ...
  def self.get_request_percentages(column, n, highest, minimum_requests)
    ...
    cis_below = []
    cis_above = []
    original_totals.each_with_index.map do |total, i|
    ...
    # percentages:
    [y_values, cis_below, cis_above].each do |l|
      l.map! { |v| 100 * v }
    end
    {
      'public_bodies' => public_bodies,
