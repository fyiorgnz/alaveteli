...
class IncomingMessage < ActiveRecord::Base
  ...
  def response_event
    info_request_events.detect { |e| e.event_type == 'response' }
  end

  # Return a cached structured mail object
  def mail(force = nil)
    if (!force.nil? || @mail.nil?) && !raw_email.nil?
      @mail = MailHandler.mail_from_raw_email(raw_email.data)
    end
    @mail
  end
  ...
  def _calculate_valid_to_reply_to
    ...
    return false unless MailHandler.get_auto_submitted(mail).nil?
    true
  end
  ...
  def parse_raw_email!(force = nil)
    ...
    # parsing bugs, etc).
    fail "Incoming message id=#{id} has no raw_email" if raw_email.nil?
    if !force.nil? || last_parsed.nil?
      ActiveRecord::Base.transaction do
        self.extract_attachments!
        self[:sent_at] = mail.date || created_at
        self[:subject] = mail.subject
        self[:mail_from] = MailHandler.get_from_name(mail)
        if from_email
          self.mail_from_domain = PublicBody.extract_domain_from_email(from_email)
        else
          self.mail_from_domain = ''
        end
        self[:valid_to_reply_to] = _calculate_valid_to_reply_to
        self.last_parsed = Time.now
        foi_attachments reload = true
        self.save!
      end
    end
  end
  ...
  def _cache_main_body_text
    ...
    if text.size > 1_000_000 # 1 MB ish
      fail 'main body text more than 1 MB, need to implement clipping like for attachment text, or there is some other MIME decoding problem or similar'
    end
  ...
  def extract_attachments!
    ...
    attachment_attributes.each do |attrs|
      attachment = foi_attachments.find_or_create_by_hexdigest(attrs[:hexdigest])
  ...
  def get_body_for_quoting
    ...
    text.strip!
    fail 'internal error' if text.nil?
    text
  end
  ...
  def get_attachment_text_clipped
    if cached_attachment_text_clipped.nil?
      ...
      fail 'internal error' if cached_attachment_text_clipped.nil?
    end
  ...
  def fully_destroy
    ActiveRecord::Base.transaction do
      ...
      info_request_events.each do |info_request_event|
        ...
        info_request_event.user_info_request_sent_alerts.each(&:destroy)
  ...
  def self.find_all_unknown_mime_types
    for incoming_message in IncomingMessage.find(:all)
      for attachment in incoming_message.get_attachments_for_display
        fail 'internal error incoming_message ' + incoming_message.id.to_s if attachment.content_type.nil?
  ...
  def self.get_all_file_extensions
    AlaveteliFileTypes.all_extensions.join(' ')
  end
