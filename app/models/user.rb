...
class User < ActiveRecord::Base
  ...
  validates_inclusion_of :admin_level, in: %w(none super), message: N_('Admin level is not included in list')
  ...
  def self.find_user_by_email(email)
    find(:first, conditions: ['lower(email) = lower(?)', email])
  end
  ...
  def reindex_referencing_models
    ...
    if changes.include?('url_name')
      comments.each do |comment|
        comment.info_request_events.each(&:xapian_mark_needs_index)
      end

      info_requests.each do |info_request|
        info_request.info_request_events.each(&:xapian_mark_needs_index)
      end
    end
  end
  ...
  def update_url_name
    ...
    until User.find_by_url_name(unique_url_name, conditions: id.nil? ? nil : ['id <> ?', id]).nil?
  ...
  def public_banned?
    warn '[DEPRECATION] User#public_banned? will be replaced with
  ...
  def set_defaults
    self.admin_level = 'none' if admin_level.nil?
    if new_record?
  ...
  def purge_in_cache
    info_requests.each(&:purge_in_cache) if name_changed?
  end
end
