...
module ActsAsXapian
  ...
  if $acts_as_xapian_class_var_init
    fail 'The acts_as_xapian module has already been loaded'
  else
  ...
  def self.prepare_environment
    ...
    fail 'Set RAILS_ENV, so acts_as_xapian can find the right Xapian database' unless environment
  ...
  def self.readable_init
    fail NoXapianRubyBindingsError.new('Xapian Ruby bindings not installed') unless ActsAsXapian.bindings_available
    fail "acts_as_xapian hasn't been called in any models" if @@init_values.empty?
    ...
    for init_value_pair in @@init_values
      ...
      if options[:terms]
        for term in options[:terms]
          fail 'Use a single capital letter for term code' unless term[1].match(/^[A-Z]$/)
          fail 'M and I are reserved for use as the model/id term' if term[1] == 'M' or term[1] == 'I'
          fail 'model and modelid are reserved for use as the model/id prefixes' if term[2] == 'model' or term[2] == 'modelid'
          fail 'Z is reserved for stemming terms' if term[1] == 'Z'
          fail "Already have code '" + term[1] + "' in another model but with different prefix '" + @@terms_by_capital[term[1]] + "'" if @@terms_by_capital.include?(term[1]) && @@terms_by_capital[term[1]] != term[2]
      ...
      if options[:values]
        for value in options[:values]
          fail "Value index '" + value[1].to_s + "' must be an integer, is " + value[1].class.to_s if value[1].class != 1.class
          fail "Already have value index '" + value[1].to_s + "' in another model but with different prefix '" + @@values_by_number[value[1]].to_s + "'" if @@values_by_number.include?(value[1]) && @@values_by_number[value[1]] != value[2]

          # date types are special, mark them so the first model they're seen for
          unless @@values_by_number.include?(value[1])
            if value[3] == :date
              ...
              fail "Unknown value type '" + value[3].to_s + "'"
            end
  ...
  class QueryBase
    ...
    def initialize_db
      ...
      fail 'ActsAsXapian not initialized' if ActsAsXapian.db.nil?
    end
    ...
    def initialize_query(options)
      ...
      self.runtime += Benchmark.realtime {
        ...
        limit = options[:limit]
        fail 'please specifiy maximum number of results to return with parameter :limit' unless limit
        limit = limit.to_i
        ...
        if sort_by_prefix.nil?
          ...
          fail "couldn't find prefix '" + sort_by_prefix.to_s + "'" if value.nil?
        ...
        if collapse_by_prefix.nil?
          ...
          fail "couldn't find prefix '" + collapse_by_prefix + "'" if value.nil?
    ...
    delegate :matches_estimated, to: :matches
  ...
  class Search < QueryBase
    ...
    def initialize(model_classes, query_string, options = {}, user_query = nil)
      ...
      for model_class in model_classes
        fail 'pass in the model class itself, or a string containing its name' if model_class.class != Class && model_class.class != String
  ...
  def self.update_index(flush = false, verbose = false)
    ...
    if File.exist?(new_path)
      fail 'aborting incremental index update while full index rebuild happens; found existing ' + new_path
    end

    ids_to_refresh = ActsAsXapianJob.find(:all).map(&:id)
    for id in ids_to_refresh
      ...
      begin
        ActiveRecord::Base.transaction do
          ...
          STDOUT.puts("ActsAsXapian.update_index #{job.action} #{job.model} #{job.model_id} #{Time.now}") if verbose
          ...
          begin
            if job.action == 'update'
              ...
              fail "unknown ActsAsXapianJob action '" + job.action + "'"
            end
        rescue ActiveRecord::RecordNotFound => e
      ...
      STDERR.puts(detail.backtrace.join("\n") + "\nFAILED ActsAsXapian.update_index job #{id} #{$ERROR_INFO} " + (job.nil? ? '' : 'model ' + job.model + ' id ' + job.model_id.to_s))
      end
    end
  ...
  def self.rebuild_index(model_classes, verbose = false, terms = true, values = true, texts = true, safe_rebuild = true)
    ...
    if File.exist?(new_path)
      fail 'found existing ' + new_path + ' which is not Xapian flint database, please delete for me' unless ActsAsXapian._is_xapian_db(new_path)
    ...
    if safe_rebuild
      ...
      for model_class in model_classes
        STDOUT.puts("ActsAsXapian.rebuild_index: Rebuilding #{model_class}") if verbose
    ...
    if File.exist?(temp_path)
      @@db_path = old_path
      fail 'temporary database found ' + temp_path + ' which is not Xapian flint database, please delete for me' unless ActsAsXapian._is_xapian_db(temp_path)
    ...
    if File.exist?(temp_path)
      unless ActsAsXapian._is_xapian_db(temp_path)
        @@db_path = old_path
        fail 'old database now at ' + temp_path + ' is not Xapian flint database, please delete for me'
      end
      FileUtils.rm_r(temp_path)
    end
  ...
  def self._rebuild_index_safely(model_classes, verbose, terms, values, texts)
    ...
    for model_class in model_classes
      ...
      0.step(model_class_count, batch_size) do |i|
        ...
        if pid
          Process.waitpid(pid)
          fail 'batch fork child failed, exiting also' unless $CHILD_STATUS.success?
        # database connection doesn't survive a fork, rebuild it
          ...
          @@db_path = ActsAsXapian.db_path + '.new'
          ActsAsXapian.writable_init
          STDOUT.puts("ActsAsXapian.rebuild_index: New batch. #{model_class} from #{i} to #{i + batch_size} of #{model_class_count} pid #{Process.pid}") if verbose
  ...
  module InstanceMethods
    ...
    def xapian_value(field, type = nil, index_translations = false)
      if index_translations && self.respond_to?('translations')
        if type == :date or type == :boolean
          ...
          values = []
          for locale in translations.map(&:locale)
    ...
    def single_xapian_value(field, type = nil)
      ...
      if type == :date
        if value.is_a?(Time)
          ...
          fail 'Only Time or Date types supported by acts_as_xapian for :date fields, got ' + value.class.to_s
        end
      elsif type == :boolean
        value ? true : false
      else
    ...
    def xapian_mark_needs_index_if_reindex
      return true if self.respond_to?(:no_xapian_reindex) && no_xapian_reindex == true
    ...
    def xapian_before_create_job_hook(_action, _model, _model_id)
    end
   end
